...
object RailwayScheduleDetailsDF {
  ...
  def main(args: Array[String]): Unit = {
    ...
    //     Train Details
    val finalTrainDetailsDF = parseTrainDetails(trainSchedulesWithStateDetails)

    println("Final train and schedule related details processed successfully!!!")

    if (!finalTrainDetailsDF.head(1).isEmpty) {
      finalTrainDetailsDF.write.mode(SaveMode.Overwrite)
        //      .partitionBy("state")
        .option("header", "true")
        .csv(s"${ROOT_FOLDER}/target/actual_schedules")
      finalTrainDetailsDF.show(false)
      println(s"FinalDF total count => ${finalTrainDetailsDF.count()} records")
      println("Final train schedule details persisted successfully as CSV format and partitioned by state!!!")
    } else {
      throw new Exception("No records are available to persist!")
    }
  }
  ...
  def parseTrainDetails(trainSchedulesWithStateDetails: DataFrame): Dataset[Row] = {
    ...
    val noneValueUDF = udf((value: String) => if (value == "None") "00:00:00" else value)
    ...
    trainSchedulesWithStateDetails
      ...
      .select(cols: _*)
      .withColumn("arrival", noneValueUDF(col("arrival")))
      .withColumn("departure", noneValueUDF(col("departure")))
  }
